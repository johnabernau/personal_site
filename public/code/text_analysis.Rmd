...
```{r}
# Generate some respondent ids and (identical) text responses
...
Note that by removing punctuation, "textcleaning" was collapsed into one word. This is usually acceptable for most hyphenated words, but in case you want to preserve the spacing, you can just replace all punctuation with a space (" ") and the subsequent line will strip / collapse any extra spaces that result. 
...
Then I create a separate dataframe that captures a few summary measures (total, unique, and lexical diversity). This will come in handy later on.
```{r}
...
Saving this tidy dataframe as new strings by collapsing words [(thanks to stackoverflow)](https://stackoverflow.com/questions/46734501/opposite-of-unnest-tokens). This will come in handy later on. 
```{r}
...
**Basic Sentiment Scores**  
To get the dictionary-based sentiment results, I first run an `inner-join` which retains only the "emotion words" in the `bing` dictionary. Then, I do calculate two things: (1) a string of the positive / negative words that were extracted, and (2) a count of the total emotion words extracted and the proportion positive. 
```{r, message = FALSE}
...
<img src="/code/hm2.jpg" style="float: left; width: 49%; margin-right: 1%; margin-bottom: 3%;">
<img src="/code/dwsb.jpg" style="float: right; width: 49%; margin-right: 1%; margin-bottom: 3%;">
