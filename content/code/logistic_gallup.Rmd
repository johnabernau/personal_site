...
If you're like me, estimates of the log-odds don't mean much. By exponentiating our coefficients (and confidence intervals), we can create odds ratios for easier interpretation. Taking `(odds ratio - 1) * 100` I calculate the percentage change. We can also add a new term `sig` for whether our confidence intervals overlap with 1 (our null value in terms of odds ratios).
...
The following plot presents our coefficient estimates and confidence intervals, in terms of odds ratios. I also color by our new variable `sig` to highlight estimates that don't overlap with our null value. PS - The intercept estimate throws off the scale of our plot, so I constrain the x-axis to a reasonable range. 
```{r}
...
Plotting coefficients in terms of percentage change. This plot also adds a text label of the coefficient estimate, provided the confidence interval doesn't overlap the null value. 
